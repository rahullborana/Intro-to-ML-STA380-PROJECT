...
###Answer
First we read the text file given and convert it into "transaction" class from
```{r}
...
```
As we can see, Whole Milk is the most common item occuring in more than 25% of the transactions. Also let's take the support as 0.025 which is near the mean support. Now we'll look for association rules by setting initial parameters as 2 for min-len and 0.3 for confidence.
```{r}
...
1. Whenever pip fruit is bought, whole vegetable is likely bought too.
2. If people purchase root vegetables or citrus fruits, they are likely to purchase other vegetables too.
3. Whenever people purchase tropical fruits, they likely purchase other vegetables.
Support 0.01 and confidence 0.5 is chosen as the top 10 items provide a good lift between 2 to 3.
